"""Streamlit presentation and session state; business rules live in ekt."""
import pandas as pd
import streamlit as st
from ekt.demo import demo_bundle, canonical_zip, DEMO_DATE
from ekt.schema import PROVENANCE, normalize, validate, fingerprint
from ekt.application import import_canonical_files, replace_supplier, edit_table
        ...
        if source == "Демонстрация":
            ...
            if import_kind == "Отчёты IEK / Systeme":
                ...
                if st.button("Импортировать отчёты", disabled=partner_upload is None, width="stretch"):
                    try:
                        ...
                        load_bundle(replace_supplier(previous, incoming, supplier))
            ...
            if st.button("Загрузить файлы", width="stretch", disabled=not files):
                try:
                    ...
                    combined = import_canonical_files(((uploaded.name, uploaded.getvalue()) for uploaded in files), mode, previous)
