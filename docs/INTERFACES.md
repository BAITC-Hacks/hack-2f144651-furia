...
- `validate(bundle) -> list[str]`: пустой список означает отсутствие обнаруженных
  ошибок схемы, а не полноту всех бизнес-входов. Ссылки проверяются членством
  составного supplier/SKU в ключах products без размножения строк;
  диагностика дубликатов products сохраняется.
...
- `ingest.read_canonical(data: bytes, filename: str, mode="manual", budget=None) -> Bundle`:
...
- `application.import_canonical_files(files, mode="manual", previous=None, budget=None)`:
  общий бюджет всей пачки `(filename, bytes)`; ошибка не меняет previous.
- `ingest.ImportBudget(...)`: необязательный общий счётчик, по умолчанию
  100 МиБ входных байтов, 200 МиБ распакованных ZIP/XLSX, 100 файлов,
  500 000 строк, 5 000 000 ячеек. XLSX проверяется до загрузки книги;
  учитываются все листы и прямоугольные размеры. Превышение — ValueError.
  Индивидуальные ограничения файла/архива 100 МиБ сохраняются.
  CSV/XLSX одинаково сохраняют буквальные NA/NULL/N/A и строковые коды.
...
- `partner.read_partner(data, filename, supplier, report_date,
  confirmed_scope=None, iek_moq_meaning="unknown", inbound_base_units=False, budget=None) -> Bundle`.
...
Только XLSX-копия заменяет XML-недопустимые C0 символы видимым `\xNN`.
Текст длиннее 32767 символов после преобразования или непредставимый Unicode
даёт ValueError с координатами; данные молча не обрезаются. То же правило
применяется к заголовкам и метаданным. Frontend должен перехватывать ошибку
непосредственно вокруг подготовки `xlsx_bytes` перед download_button (AUD-FE-01).
